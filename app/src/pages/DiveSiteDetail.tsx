...
import { useSiteCommunityStats } from '../hooks/useCommunityStats'
...
export function DiveSiteDetail() {
  ...
  const community = useSiteCommunityStats(siteName)
  ...
  return (
    <div class="px-4 py-4 pb-6">
      ...
      <div class="bg-white rounded-2xl p-4 shadow-sm mt-3">
        ...
        {community.offline ? (
        ...
        ) : community.data && (community.data.total_dives > 0 || community.data.unique_divers > 0) ? (
          <>
            <div class="grid grid-cols-3 gap-2">
              <div class="text-center p-2 bg-ocean-50 rounded-xl">
                <div class="text-lg font-bold text-ocean-800">{community.data.total_dives}</div>
              ...
              <div class="text-center p-2 bg-ocean-50 rounded-xl">
                <div class="text-lg font-bold text-ocean-800">{community.data.species_count}</div>
                <div class="text-[10px] text-ocean-500">{t('community.species_count')}</div>
              </div>
            </div>

            {community.data.top_species.length > 0 && (
              <div class="mt-3">
                <p class="text-[10px] text-ocean-400 uppercase tracking-wide mb-2">{t('community.top_species')}</p>
                <div class="space-y-2">
                  {community.data.top_species.slice(0, 5).map((item, i) => {
                      ...
                      >
                        ...
                        <div class="flex-1 min-w-0">
                          ...
                          <p class="text-[10px] text-ocean-400">
                            {item.count} {t('community.times_spotted')}
                          </p>
                        </div>
                      </a>
                    )
          ...
          <p class="text-xs text-ocean-400">{t('community.no_data')}</p>
        )}
      </div>
